    ...
    public function show(Request $request, $id)
{
    ...
    try {
        DB::statement('CALL sp_RegistrarAuditoria(?, ?, ?, ?, ?, ?, ?, ?, ?)', [
            'Barberos', $barbero->IdBarbero, 'SELECT', 'VerPerfil',
            null, null, $admin->IdUsuario, $ip, 'Admin consultó perfil de barbero'
        ]);
    } catch (\Exception $e) {}

    // Horarios del barbero desde HorariosBarberos + Horarios
    $horariosBarbero = HorarioSemanal::where('IdBarbero', $barbero->IdBarbero)
        ->where('EstadoA', 1)
        ->with('horario')
        ->get();
    ...
    $horarios = $horariosBarbero->map(function ($hb) {
        return [
            'dia_semana'   => $hb->horario->DiaSemana   ?? null,
    ...
    /**
     * HU-02 Escenario 4: Reactivar/Activar barbero.
     */
    public function activar(Request $request, $id)
    {
        ...
        try {
            DB::statement('CALL sp_ActivarBarbero(?, ?, ?)', [
                $barbero->IdBarbero,
                $admin->IdUsuario,
                $ip,
            ]);
            ...
            return response()->json([
                'mensaje' => 'Error al activar el barbero',
                'error' => $e->getMessage(),
            ], 500);
        }

        return response()->json([
            'mensaje' => 'Barbero activado correctamente',
        ], 200);
    }
}