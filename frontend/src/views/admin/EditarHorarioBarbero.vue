...
const idHorarioSemanal = ref(null)
...
async function guardar() {
  ...
  try {
    if (idHorarioSemanal.value) {
      // Actualizar horario existente
      await barberoService.actualizarHorario(idHorarioSemanal.value, diasActivos)
    } else {
      // Crear horario nuevo
      const semanaActual = new Date()
      await barberoService.crearHorario({
        id_barbero: idBarbero,
        semana:     getWeekNumber(semanaActual),
        ano:        semanaActual.getFullYear(),
        dias:       diasActivos,
      })
    }
...
onMounted(async () => {
  try {
    ...
    if (horario) {
      idHorarioSemanal.value = horario.id_horario_semanal
