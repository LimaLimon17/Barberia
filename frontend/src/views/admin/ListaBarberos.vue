...
onMounted(async () => {
  await cargarBarberos(true)
})

// Añadimos 'mostrarSpinner' para evitar que la tabla desaparezca bruscamente al actualizar
async function cargarBarberos(mostrarSpinner = true) {
...
async function desactivar() {
  ...
  try {
    ...
    // Recargamos los datos en segundo plano sin mostrar el spinner de carga completa
    await cargarBarberos(false)
...
async function habilitar(barbero) {
  error.value = ''
  try {
    // Log para verificar en la consola del navegador que el ID no vaya undefined
    console.log("Intentando habilitar al barbero con ID:", barbero.id_barbero)

    // Asegúrate de que este método se llame 'activar' dentro de tu barberoService.js
    await barberoService.activar(barbero.id_barbero)

    mensajeExito.value = `${barbero.nombre_completo} fue habilitado correctamente.`

    // Recargamos los datos en segundo plano
    await cargarBarberos(false)
    ...
    error.value = err.response?.data?.mensaje || 'Error al habilitar el barbero'
  }
}
</script>
